...
!
implicit none
integer, parameter :: n=100,nx=n,ny=1,nz=1   !1D for now
real, dimension(5,nx,ny,nz) :: u  !fluid variables
real, dimension(3,nx,ny,nz) :: b  !magnetic field
real t,dt,tf
integer iter,i
...
! x again
   ...
   call advectbyzx(u,b,nx,ny,nz,dt)
   call fluidx(u,b,nx,ny,nz,dt)
   !Strang type splitting for 2nd order accuracy
   
   !if (mod(iter,10) .eq. 1) 
   write(*,*) 'timestep,dt=',iter,dt!,u(2,nx/4,1,1)
   call output
end do
!call output
close(20)
...
subroutine init(u,b,nx,ny,nz)
  ...
  real p0,pL,pR,rhoL,rhoR,uL,uR
  real vL,vR,wL,wR,bxL,bxR,byL,byR,bzL,bzR
  integer i
...
!  return
  ...
  wR=0.
  u=0
  !b=2./sqrt(4.*4.*atan(1.d0))
  bxL=2./sqrt(4.*4.*atan(1.d0))
  bxR=2./sqrt(4.*4.*atan(1.d0))
  bzL=2./sqrt(4.*4.*atan(1.d0))
  bzR=2./sqrt(4.*4.*atan(1.d0))
  byL=3.6/sqrt(4.*4.*atan(1.d0))
  byR=4./sqrt(4.*4.*atan(1.d0))
  
  !b(1,:,:,:)=2./sqrt(4.*4.*atan(1.d0))
  !b(3,:,:,:)=2./sqrt(4.*4.*atan(1.d0))
  !u(1,:,:,:)=1
  !u(5,:,:,:)=p0*1.5
  do i=1,nx
     if(i<nx/2)then
...
subroutine advectbyzx(u,b,nx,ny,nz,dt)
  ...
  do k=1,nz
     do j=1,ny
        jm=-mod(j+ny-2,ny)+1!mod(j+ny-2,ny)+1
  ...
  do k=1,nz
     ...
     do j=1,ny
        km=-mod(k+nz-2,nz)+1!mod(k+nz-2,nz)+1
