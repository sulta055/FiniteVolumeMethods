...
!Subroutine List:
!
!massTracerInit(u1,dx,dy),tracerDensity(xmin,ymin,dx,dy),
!cellTransfer(ix0,iy0,ix1,iy1,N_out),massTracerAdvect(ix,iy,fL,fR,rho,dt,dr,dir) 
!--------------------------------------------------------------------------------
...
integer ::N_cell(0:nx+1,0:ny+1) !number of tracers in a given cell
!temp variables
integer ::N_cell_temp(0:nx+1,0:ny+1)
integer ::temp_id
...
!--------------------------------------------------------------------
subroutine massTracerInit(u1,dx,dy)
!--------------------------------------------------------------------
!Mass Tracer Initialization Routine

!Input Variables
real*8::dx,dy,u1(-1:nx+2,-1:ny+2,5)
...
!N_out tracers in the cell_(ix0,iy0) link are going to be moved into cell_(ix1,iy1).
...
if(ix1>nx+1 .or. ix1<0 .or. iy1>ny+1 .or. iy1<0) then
 ...
 do kk=1,N_out
   ...
   if(tr_counter2>1 .and. tr_counter2<=tr_counter+1) then 
     ...
     !print*,'Cell#',ix,'head tracer: ',associated(cell_tracer_head(ix)%p)
     
      
     !print*,'ix,N_cell=',ix!,N_cell_temp(ix)    
