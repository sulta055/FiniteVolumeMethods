...
if(uniformAdvection_test==1)then
...
do jj=-1,nx+2
 do kk=-1,ny+2
  if((jj-nx/2)*(jj-nx/2)+(kk-ny/2)*(kk-ny/2)<=(nx/14)*(nx/14)) then
    u1(jj,kk,1)=rhoL
    u1(jj,kk,2)=rhoL*uL
    u1(jj,kk,3)=rhoL*vL
    u1(jj,kk,4)=rhoL*wL
...
do jj=-1,nx+2
 do kk=-1,ny+2
  if(kk>0.49*ny .and. kk<0.51*ny .and. jj<=1) then 
    u1(jj,kk,1)=rhoR
    u1(jj,kk,2)=rhoR*uR
    u1(jj,kk,3)=rhoR*vR
    u1(jj,kk,4)=rhoR*wR
...
do jj=1,nx
  ...
  if(logOutput==0)then
    write(12,*) x,dens,velx,pres,real(cell_count(jj,ycell))*tracer_mass/(dx*dy)
  else if (logOutput==1)then
    if(cell_count(jj,ycell)>0)then
     write(12,*) x,log(dens),velx,pres,log(real(cell_count(jj,ycell))*tracer_mass/(dx*dy)),pres
    else
     write(12,*) x,log(dens),velx,pres,real(cell_count(jj,ycell))*tracer_mass/(dx*dy),pres
    end if
  end if
...
do jj=1,nx
  ...
  pres=(gam-1.)*(u1(xcell,jj,8)-0.5*dens*(velx**2.+vely**2.+velz**2.)&
        -0.5*(magx**2.+magy**2.+magz**2.))   
  if(logOutput==0)then
    write(14,*) y,dens,velx,vely,cell_count(xcell,jj),pres
  else if (logOutput==1)then
    if(cell_count(xcell,jj)>0)then
     write(14,*) y,log(dens),velx,vely,log(real(cell_count(xcell,jj))),pres
    else
     write(14,*) y,log(dens),velx,vely,cell_count(xcell,jj),pres
    end if
  end if
...
do kk=1,ny
  ...
  do jj=1,nx
   ...
   if(logOutput==0)then
    write(iunit,*) x,y,dens,divU(jj,kk),real(cell_count(jj,kk))*tracer_mass/(dx*dy)!,vely,pres,magpres
   else if(logOutput==1)then
    if(cell_count(jj,kk)>0)then
     write(iunit,*) x,y,log(dens),divU(jj,kk),log(real(cell_count(jj,kk))*tracer_mass/(dx*dy))
    else
     write(iunit,*) x,y,log(dens),divU(jj,kk),real(cell_count(jj,kk))*tracer_mass/(dx*dy)
    end if
   end if
...
if(jet_test==1)then
 do kk=-1,ny+2
  if(kk>0.49*ny .and. kk<0.51*ny) then !ambient medium
   u2(-1,kk,2)=1.5
   u2(0,kk,2)=1.5
   u2(-1,kk,3)=0.
   u2(0,kk,3)=0.
...
subroutine findShock() !shock finder routine

integer::ii,jj,kk     
real*8::pL,pR,pU,pD,p0,rhoL,rhoR,rhoU,rhoD,rho0,r
real*8::u0,v0,uL,uR,uU,uD,vL,vR,vU,vD

shockCell=0.
do jj=1,ny
 do ii=1,nx
  !check velocity gradient (divU<0 inside shock cell)
  if(divU(ii,jj)<1.d-25)then
   !check pressure and density jumps (compare with 4 adjacent cells)
   rho0=u1(ii,jj,1)
   rhoL=u1(ii-1,jj,1)
   rhoR=u1(ii+1,jj,1)
   rhoD=u1(ii,jj-1,1)
   rhoU=u1(ii,jj+1,1)
   !u0=u1(ii,jj,2)/u1(ii,jj,1)
   !v0=u1(ii,jj,3)/u1(ii,jj,1)

   !shock compression ratio
   r=max(rhoL/rho0,rho0/rhoL,rhoR/rho0,rho0/rhoR,&
     rhoD/rho0,rho0/rhoD,rhoU/rho0,rho0/rhoU,&
     rhoL/rhoR,rhoR/rhoL,rhoD/rhoU,rhoU/rhoD)
