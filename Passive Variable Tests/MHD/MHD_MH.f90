...
do i=1,nt
  ...
  !Mass tracer injection
  if(tracerType==1)then
   if(trInject==1)then
    do j=1,4 
    do k=1,5
     call massTracerInject(1,98+k-1)
    end do
    end do
   end if
  end if
  ...
  if(randforce_test==1 .and. i<0.4*nt)then
   !Add Contribution from random force to fluid momentum and kinetic energy
   do j=1,nx,2
   do k=1,ny,2
    u2(j,k,8)=u2(j,k,8)-0.5*(u2(j,k,2)**2.+u2(j,k,3)**2.+u2(j,k,4)**2.)/u2(j,k,1) 
    u2(j,k,2)=u2(j,k,2)+0.01*(rand(0)-0.5) !x-momentum
    u2(j,k,3)=u2(j,k,3)+0.01*(rand(0)-0.5) !y-momentum
    u2(j,k,8)=u2(j,k,8)+0.5*(u2(j,k,2)**2.+u2(j,k,3)**2.+u2(j,k,4)**2.)/u2(j,k,1) !total energy
   end do
   end do
  end if
  ...
  if(tracerType==2)then
   if(trInject==1)then
    do j=1,4 
     do k=1,5
      call velTracerInject(1,98+k-1)
     end do
    end do
   end if
   call velTracerAdvect(dt)
   !call velTracerDiffusion(dt)
  end if
